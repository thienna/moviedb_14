...
public class FetchMovieByPersonFromUrl extends BaseFetchDataFromUrl<List<Movie>> {
    private MovieDataSource.OnFetchDataListener mListener;

    public FetchMovieByPersonFromUrl(MovieDataSource.OnFetchDataListener listener) {
        mListener = listener;
    ...
    protected List<Movie> extractDataFromJson(String dataJSON) throws JSONException {
        ...
        JSONObject objectHasExactlyName = resultsArray.getJSONObject(0);
        JSONArray relatedMoviesList = objectHasExactlyName.getJSONArray(Person.JsonKey.KNOWN_FOR);
        for (int i = 0; i < relatedMoviesList.length(); i++) {
            // Current cursor
            JSONObject currentMovie = relatedMoviesList.getJSONObject(i);
